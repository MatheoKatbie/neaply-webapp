...
const createWorkflowSchema = z.object({
    ...
    shortDesc: z.string().min(10, 'Short description must be at least 10 characters').max(200, 'Short description cannot exceed 200 characters'),
    longDescMd: z.string().min(10, 'Long description must be at least 10 characters').max(5000, 'Long description cannot exceed 5000 characters').optional().or(z.literal('')),
    heroImageUrl: z.string().url('Hero image URL must be valid').optional().or(z.literal('')),
    basePriceCents: z.number().min(100, 'Base price must be at least €1.00').max(100000, 'Base price cannot exceed €1000.00'),
...
export async function POST(req: NextRequest) {
    try {
        ...
        // TODO: Gérer les catégories et tags si fournis
        // Note: Pour l'instant, on se concentre sur le workflow de base
