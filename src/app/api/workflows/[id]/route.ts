...
const updateWorkflowSchema = z.object({
    ...
    shortDesc: z.string().min(10, 'Short description must be at least 10 characters').max(200, 'Short description cannot exceed 200 characters').optional(),
    longDescMd: z.string().min(50, 'Long description must be at least 50 characters').max(5000, 'Long description cannot exceed 5000 characters').optional(),
    heroImageUrl: z.string().url('Hero image URL must be valid').optional().or(z.literal('')),
    basePriceCents: z.number().min(100, 'Base price must be at least €1.00').max(100000, 'Base price cannot exceed €1000.00').optional(),
    ...
    try {
        ...
        if (!seller) {
            return NextResponse.json(
                { error: 'Authentication required - must be a seller' },
                { status: 401 }
            )
        }
        ...
        return NextResponse.json({ data: serializedWorkflow })
...
export async function PUT(
    ...
    try {
        ...
        // Convertir BigInt en string pour la sérialisation JSON
        const serializedUpdatedWorkflow = {
            ...updatedWorkflow,
            id: updatedWorkflow.id.toString(),
            categories: updatedWorkflow.categories.map(cat => ({
                ...cat,
                category: {
                    ...cat.category,
                    id: cat.category.id.toString()
            ...
            tags: updatedWorkflow.tags.map(tag => ({
                ...tag,
                tag: {
                    ...tag.tag,
                    id: tag.tag.id.toString()
            ...
            versions: updatedWorkflow.versions.map(version => ({
                ...version,
                id: version.id.toString(),
                workflowId: version.workflowId.toString()
            }))
        ...
        return NextResponse.json({
            data: serializedUpdatedWorkflow,
