 ...
 */
export async function getOrCreateUser(supabaseUserId: string, email: string, displayName?: string, avatarUrl?: string): Promise<DatabaseUser | null> {
    try {
 ...
 */
export async function updateUser(userId: string, updates: Partial<Pick<DatabaseUser, 'displayName' | 'avatarUrl' | 'isSeller'>>): Promise<DatabaseUser | null> {
    try {
        const response = await fetch('/api/user', {
            method: 'PATCH',
            headers: {
